...
def generate_launch_description():
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(config_dir, param_file),
    ...
    bringup_cmd_group = GroupAction([
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(launch_dir, 'rviz_view.launch.py')),
            launch_arguments={'use_sim_time': use_sim_time,
                              'open_rviz': open_rviz,
                              'map_subscribe_transient_local': 'true'}.items()),
        
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(turtlebot2_bringup_package,'launch/turtlebot_bringup.launch.py'))
        )
    ])
